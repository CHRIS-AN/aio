...
public class SlipController {
    ...
    public String searchSlip(@PageableDefault(size = 5, page = 0, sort = "slipId", direction = Sort.Direction.DESC)
...
//            Page<Slip> slipStatementManagement =
//                    slipRepository.findByKeywordAndPayStatementTypeStartDateAfter(keyword, pageable, now, now.minusMonths(months));
            Page<Slip> slipStatementManagement = slipRepository.findByPayStatementntTypeAndSlipDateBetweenAndSlipWriteContaining
                    ("승인", new SimpleDateFormat("yyyy-MM-dd").format(cal.getTime()), new SimpleDateFormat("yyyy-MM-dd").format(now), keyword, pageable);

            model.addAttribute("slipList", slipStatementManagement);
            System.out.println(slipStatementManagement.getContent());
        } else {
    ...
    public String updateStatementManagementForm(@PathVariable("slipId") String slipId, Model model){
        System.out.println("여기 들어오냐 제발 들어와라 쫌 !!!!!!!!!");
        List<Slip> slipList = new ArrayList<>();
        Slip slip = slipService.viewSlip(slipId);
        slipList.add(slip);
        model.addAttribute("slipList", slipList);
